...
$(function() {
    ...
    $(".user").on("click", result);  // 유저 가위바위보 클릭 시
});

function result() {
  var div = $(this);
  if ($("#start").prop("disabled")) { // .prop("disabled") - 비활성화면 참, 활성화면 거짓
...
function imgRoate() {
    ...
    // comIdx의 값이 2가 될 때는 화면에 보자기가 출력된 후이기 때문에 comIdx를 -1로 변경한다
    // ++comIdx에 의해 0으로 증가하여 0번 인덱스인 가위 이미지가 출력
